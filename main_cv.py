...
# hyperparameter
BATCH_SIZE = int(os.environ.get('DEEPRSMA_BATCH_SIZE', 16))
EPOCH = int(os.environ.get('DEEPRSMA_EPOCH', 1500))
NUM_WORKERS = int(os.environ.get('DEEPRSMA_NUM_WORKERS', 0 if os.name == 'nt' else 1))
hidden_dim = 128
seed_dataset = 2
seed = int(os.environ.get('DEEPRSMA_SEED', 1))
LR = 5e-4
MAX_FOLDS = int(os.environ.get('DEEPRSMA_MAX_FOLDS', 0))
MAX_SAMPLES = int(os.environ.get('DEEPRSMA_MAX_SAMPLES', 0))
N_SPLITS = int(os.environ.get('DEEPRSMA_N_SPLITS', 10))
N_REPEATS = int(os.environ.get('DEEPRSMA_N_REPEATS', 1))
...
if MAX_SAMPLES:
    rna_dataset = rna_dataset[:MAX_SAMPLES]
    molecule_dataset = molecule_dataset[:MAX_SAMPLES]
...
# 10 fold
kf = regressor_stratified_cv(n_splits=N_SPLITS, n_repeats=N_REPEATS, random_state=seed_dataset, group_count=5, strategy='uniform')
    ...
    for epo in range(EPOCH):
        ...
        with torch.set_grad_enabled(False):
            ...
            for step, (batch_rna_test,batch_mole_test) in enumerate(test_loader):
                ...
                label = batch_rna_test.y.detach().cpu().float()
