...
cold_type = os.environ.get('DEEPRSMA_COLD_TYPE', 'rna')
seed = int(os.environ.get('DEEPRSMA_SEED', 2))
LR = 5e-4
...
if MAX_SAMPLES:
    all_df = all_df.iloc[:MAX_SAMPLES].reset_index(drop=True)
...
for i, df_f in enumerate(df):
    ...
    if MAX_TRAIN:
        train_id = train_id[:MAX_TRAIN]
    if MAX_TEST:
        test_id = test_id[:MAX_TEST]
